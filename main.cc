...
template <class Archive>
void serialize(Archive & p_ar, GUID & p_guid, unsigned int const file_version) {
	p_ar & p_guid.Data1;
	p_ar & p_guid.Data2;
	p_ar & p_guid.Data3;
	p_ar & p_guid.Data4;
...
struct eqsplit_dsp : dsp_impl_base {
	...
	static optional<GUID> find_equalizer() { 
	...
	static bool g_get_default_preset(dsp_preset & p_preset) {
		optional<GUID> guid = find_equalizer();
		if(guid) {
	...
	struct split_config_callback : dsp_preset_edit_callback {
		virtual void on_preset_changed(dsp_preset const & p_new) {
			preset = p_new;
		}

		optional<dsp_preset_impl> preset;
	};
	...
	static void g_show_config_popup(dsp_preset const& p_data, HWND p_parent, dsp_preset_edit_callback & p_callback) {
		...
		if(cb[0].preset || cb[1].preset) {
			dsp_preset_impl ret;
			set_child_presets(ret,
				cb[0].preset ? *cb[0].preset : preset[0],
				cb[1].preset ? *cb[1].preset : preset[1]);
	...
	struct debug_abort_callback : abort_callback {
		explicit debug_abort_callback(abort_callback & p_abort) : cb(p_abort) { }
	...
	void split_channels(audio_chunk * p_chunk, dsp_chunk_list & cl0, dsp_chunk_list & cl1) {
		...
		for(t_size i = 0; i < 2; ++i) {
			...
			chunk[i]->set_sample_rate(sample_rate);
	...
	void merge_channels(audio_chunk * p_chunk, dsp_chunk_list & cl0, dsp_chunk_list & cl1) {
		...
		audio_sample * dst = p_chunk->get_data(),
		             * src[] = { chunk[0]->get_data(), chunk[1]->get_data() };
...
GUID const eqsplit_dsp::s_guid = 
{ 0x29213fa7, 0x160e, 0x49b9, { 0xaa, 0xf0, 0x91, 0x45, 0xa7, 0xf5, 0xf7, 0x76 } };

dsp_factory_t<eqsplit_dsp> g_asdf;

DECLARE_COMPONENT_VERSION("Split equalizer", "v0.0.1", "(c) Lars Viklund 2008")
